...
var store_ticket_coordinates = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'coordinates', 'action' => 'list_data', $ticket['Ticket']['id'])); ?>'	})
});
...
?>
		...
		var ticket_view_panel_2 = new Ext.TabPanel({
			...
			items:[
			{
				...
				loadMask: true,
				stripeRows: true,
				store: store_ticket_coordinates,
				title: '<?php __('Coordinates'); ?>',
				enableColumnMove: false,
				listeners: {
					activate: function(){
						if(store_ticket_coordinates.getCount() == '')
							store_ticket_coordinates.reload();
					}
				},
				columns: [
