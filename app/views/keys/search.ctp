...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['FromBranch.name', '<?php __('FromBranch'); ?>']
,			['ToBranch.name', '<?php __('ToBranch'); ?>']
,			['Key.key', '<?php __('Key'); ?>']
,			['Key.tt_direction', '<?php __('Tt Direction'); ?>']
,			['Key.amount_range', '<?php __('Amount Range'); ?>']
,			['Key.created', '<?php __('Created'); ?>']
,			['Key.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var keySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
...
var keySearchWindow = new Ext.Window({
	title: '<?php __('Search Key'); ?>',
	modal: true,
	items: keySearchForm,
	resizable: false,
	width: 550,
