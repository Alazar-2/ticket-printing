...
,			['IssuesRecord.created', '<?php __('Created'); ?>']
,			['Field.name', '<?php __('Field'); ?>']
,			['IssuesRecord.value', '<?php __('Value'); ?>']
		]
	}),
	mode: 'local',
...
var issuesRecordSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					issuesRecordSearchWindow.close();
...
var issuesRecordSearchWindow = new Ext.Window({
	title: '<?php __('Search IssuesRecord'); ?>',
	modal: true,
	items: issuesRecordSearchForm,
	resizable: false,
	width: 550,
	height: 400,
