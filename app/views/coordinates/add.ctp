		<?php
			$this->ExtForm->create('Coordinate');
			$this->ExtForm->defineFieldFunctions();
		?>
		var CoordinateAddForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'coordinates', 'action' => 'add')); ?>',
			defaultType: 'textfield',

			items: [
				<?php 
					$options = array();
					$this->ExtForm->input('x', $options);
				?>,
				<?php 
					$options = array();
					$this->ExtForm->input('y', $options);
				?>,
				<?php 
					$options = array();
                                          $options = array('fieldLabel' => 'x2');
					$this->ExtForm->input('length', $options);
				?>,
				<?php 
					$options = array();
                                          $options = array('fieldLabel' => 'Line');
					$this->ExtForm->input('order', $options);
				?>,
				<?php 
					$options = array();
                                       
					if(isset($parent_id))
						$options['hidden'] = $parent_id;
					else
						$options['items'] = $tickets;
					$this->ExtForm->input('ticket_id', $options);
				?>,
				<?php 
					$options = array();
					
						$options['items'] = $fields;
					$this->ExtForm->input('field_id', $options);
				?>,
				<?php 
					$options = array();
                                        $options = array('xtype' => 'combo', 'fieldLabel' => 'Alignment', 'value' => 'left');
                                        $options['items'] = array('left' => 'Left', 'right' => 'Right');
					$this->ExtForm->input('alignment', $options);
				?>			]
		});
		
		var CoordinateAddWindow = new Ext.Window({
			title: '<?php __('Add Coordinate'); ?>',
			width: 400,
			minWidth: 400,
			autoHeight: true,
			layout: 'fit',
			modal: true,
			resizable: true,
			plain:true,
			bodyStyle:'padding:5px;',
			buttonAlign:'right',
			items: CoordinateAddForm,
			tools: [{
				id: 'refresh',
				qtip: 'Reset',
				handler: function () {
					CoordinateAddForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				qtip: 'Help',
				handler: function () {
					Ext.Msg.show({
						title: 'Help',
						buttons: Ext.MessageBox.OK,
						msg: 'This form is used to insert a new Coordinate.',
						icon: Ext.MessageBox.INFO
					});
				}
			}, {
				id: 'toggle',
				qtip: 'Collapse / Expand',
				handler: function () {
					if(CoordinateAddWindow.collapsed)
						CoordinateAddWindow.expand(true);
					else
						CoordinateAddWindow.collapse(true);
				}
			}],
			buttons: [  {
				text: '<?php __('Save'); ?>',
				handler: function(btn){
					CoordinateAddForm.getForm().submit({
						waitMsg: '<?php __('Submitting your data...'); ?>',
						waitTitle: '<?php __('Wait Please...'); ?>',
						success: function(f,a){
							Ext.Msg.show({
								title: '<?php __('Success'); ?>',
								buttons: Ext.MessageBox.OK,
								msg: a.result.msg,
                                icon: Ext.MessageBox.INFO
							});
							CoordinateAddForm.getForm().reset();
<?php if(isset($parent_id)){ ?>
							RefreshParentCoordinateData();
<?php } else { ?>
							RefreshCoordinateData();
<?php } ?>
						},
						failure: function(f,a){
							Ext.Msg.show({
								title: '<?php __('Warning'); ?>',
								buttons: Ext.MessageBox.OK,
								msg: a.result.errormsg,
                                icon: Ext.MessageBox.ERROR
							});
						}
					});
				}
			}, {
				text: '<?php __('Save & Close'); ?>',
				handler: function(btn){
					CoordinateAddForm.getForm().submit({
						waitMsg: '<?php __('Submitting your data...'); ?>',
						waitTitle: '<?php __('Wait Please...'); ?>',
						success: function(f,a){
							Ext.Msg.show({
								title: '<?php __('Success'); ?>',
								buttons: Ext.MessageBox.OK,
								msg: a.result.msg,
                                icon: Ext.MessageBox.INFO
							});
							CoordinateAddWindow.close();
<?php if(isset($parent_id)){ ?>
							RefreshParentCoordinateData();
<?php } else { ?>
							RefreshCoordinateData();
<?php } ?>
						},
						failure: function(f,a){
							Ext.Msg.show({
								title: '<?php __('Warning'); ?>',
								buttons: Ext.MessageBox.OK,
								msg: a.result.errormsg,
                                icon: Ext.MessageBox.ERROR
							});
						}
					});
				}
			},{
				text: '<?php __('Cancel'); ?>',
				handler: function(btn){
					CoordinateAddWindow.close();
				}
			}]
		});
